...
function [ top ] = conv( bottom, weight, bias, K, S, pad, group )
    [Win,Hin,N]=size(bottom);%es el numero de canales por imagen
    [~,~,~,M]=size(weight);%num de filtros
    ...
    switch group
        ...
        case 2,
            ...
            for w=1:Wout
                for h=1:Hout
                    for m=1:M/2 %la mitad de los filtros se convoluaciona con la mitad de los canales de entrada 1 128
                        wStart=(w-1)*S+1;
                        wEnd=wStart+K-1;
                        hStart=(h-1)*S+1;
                        hEnd=hStart+K-1;
